...
if (bhaCanvas) {
  ...
  function hitTest(it, x, y) {
    ...
    const bbMinX = it.x + b.minX * scale;
    const bbMaxX = it.x + b.maxX * scale;
    const bbMinY = it.y + b.minY * scale;
    const bbMaxY = it.y + b.maxY * scale;

    for (const p of it.comp.parts) {
      const pts = partPolygonPoints(p, 0, 0).map(pt => {
        const px = it.x + (it.flipped ? b.width - pt.x : pt.x) * scale;
        const py = it.y + (it.flipped ? b.height - pt.y : pt.y) * scale;
        return { x: px, y: py };
      });
